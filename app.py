...
def apply_global_style() -> None:
    """20대 감성의 트렌디한 그라디언트 테마 + 부드러운 애니메이션을 적용한다."""
    ...
    card_bg = "linear-gradient(145deg, #23232f, #1c1c26)" if is_dark else "linear-gradient(145deg, #ffffff, #f7f5ff)"
    border_color = "#3a3b45" if is_dark else "#e9e5ff"
    text_color = "#f2f2f5" if is_dark else "#1a1a1a"
    word_color = "#7c9dff" if is_dark else "#6d5ef2"
    accent2_color = "#c084fc" if is_dark else "#8b5cf6"
    hint_bg = "#2b2b36" if is_dark else "#f4f1ff"
    hint_text = "#e8e8e8" if is_dark else "#3d3d4a"
    ans_color = "#4ade80" if is_dark else "#16a34a"
    shadow_color = "rgba(124,58,237,0.22)" if is_dark else "rgba(124,58,237,0.14)"
    muted_color = "#9a9aa2" if is_dark else "#6b7280"
    accent_bg = "#241f38" if is_dark else "#eef0ff"
    accent_bg2 = "#2d1f3d" if is_dark else "#f5ecff"
    accent_text = "#b9a6ff" if is_dark else "#5b3fd6"
    ...
    st.markdown(f"""
        ...
        @keyframes floatGlow {{
            0%, 100% {{ box-shadow: 0 4px 18px {shadow_color}; }}
            50% {{ box-shadow: 0 10px 30px {shadow_color}; }}
        }}
        ...
        .study-card {{
            ...
            animation: fadeInUp 0.45s cubic-bezier(0.22, 1, 0.36, 1) both, floatGlow 3.2s ease-in-out infinite;
        }}
        .word-text {{
            font-size: {large}px !important; font-weight: 800; margin-bottom: 8px; text-align: center;
            background: linear-gradient(90deg, {word_color}, {accent2_color}, {word_color});
        ...
        .hint-box {{
            ...
            padding: 12px 14px; border-radius: 14px; margin-top: 12px; line-height: 1.6;
            animation: popIn 0.3s ease both;
        }}
        .qa-compact {{ padding: 22px 20px !important; margin: 10px 0 !important; }}
        .qa-compact .test-question {{ padding: 6px 4px 4px 4px !important; }}
        .qa-compact .test-answer {{ margin: 2px 0 6px 0 !important; }}
        .qa-compact .hint-box {{ margin-top: 8px !important; }}
        ...
        .test-question {{
            font-size: {huge}px !important; color: {text_color}; text-align: center; padding: 26px 10px 10px 10px;
            font-weight: 800; word-break: break-word; animation: fadeInUp 0.35s ease both;
        }}
        .test-answer {{
            font-size: {large}px !important; text-align: center; color: {ans_color}; font-weight: 700; margin: 4px 0 20px 0;
            animation: popIn 0.35s cubic-bezier(0.34, 1.56, 0.64, 1) both;
        }}

        .progress-caption {{ text-align: center; color: {muted_color}; margin-top: 6px; font-weight: 500; }}

        .active-files-box {{
            background: linear-gradient(135deg, {accent_bg}, {accent_bg2});
            color: {accent_text};
            border-radius: 16px;
            padding: 12px 16px;
            margin: 10px 0 4px 0;
            font-size: {int(base*0.9)}px;
            line-height: 1.6;
            word-break: break-all;
            animation: fadeInUp 0.4s ease both;
        }}
        .active-files-box b {{ color: {accent_text}; }}

        div[data-testid="stButton"] > button {{
            font-size: {base}px !important; font-weight: 700 !important; border-radius: 12px !important;
            transition: transform 0.15s ease, box-shadow 0.15s ease !important;
        }}
        div[data-testid="stButton"] > button:hover {{
            transform: translateY(-2px) scale(1.02);
            box-shadow: 0 8px 18px {shadow_color};
        }}
        ...
        div[data-testid="stProgress"] div[role="progressbar"] > div {{
            ...
            background: linear-gradient(90deg, {word_color}, {accent2_color}) !important;
        }}

        div[data-testid="stCheckbox"] {{ display: flex; justify-content: center; }}

        .file-check-row div[data-testid="stCheckbox"] {{
            justify-content: flex-start !important;
        }}
        .file-check-row div[data-testid="stCheckbox"] label p {{
            white-space: nowrap !important;
            overflow-x: auto !important;
            font-size: {int(base*0.92)}px !important;
        }}
        ...
        .bg-blob-1 {{ top: -60px; left: -60px; width: 240px; height: 240px; background: {word_color}; }}
        .bg-blob-2 {{ top: 40%; right: -80px; width: 260px; height: 260px; background: {accent2_color}; animation-delay: 2s; }}
        ...
        .stCaption, [data-testid="stCaptionContainer"] {{ opacity: 0.85; }}

        /* ---------- 컴팩트 달력 그리드: 모든 화면 크기에서 7칸 유지 ---------- */
        .st-key-calendar_grid_box div[data-testid="stHorizontalBlock"] {{
            flex-wrap: nowrap !important;
            gap: 2px !important;
        }}
        .st-key-calendar_grid_box div[data-testid="column"] {{
            min-width: 0 !important;
            width: 14.28% !important;
            flex: 1 1 0 !important;
            padding: 0 1px !important;
        }}
        .st-key-calendar_grid_box div[data-testid="stButton"] button {{
        ...
        @media (max-width: 640px) {{
            ...
            div[data-testid="stButton"] > button {{
                ...
                font-size: {max(12, int(base * 0.85))}px !important;
                white-space: nowrap;
            }}
...
def mobile_stack_container(key: str):
    """이 컨테이너 안의 버튼 행은 모바일 화면에서 세로로 쌓인다."""
    box = st.container(key=key)
    box.markdown(
        f'<style>.st-key-{key} {{ }} </style>'
        f'<div class="mobile-stack-marker"></div>',
        unsafe_allow_html=True
    )
    return box
...
def fire_confetti() -> None:
    """canvas-confetti(JS) 라이브러리를 이용해 완료 시 컨페티 애니메이션을 터뜨린다."""
    components.html("""
        <script src="https://cdn.jsdelivr.net/npm/canvas-confetti@1.9.2/dist/confetti.browser.min.js"></script>
        <script>
        (function() {
            function launch() {
                if (typeof confetti !== 'function') { setTimeout(launch, 100); return; }
                confetti({
                    particleCount: 90,
                    spread: 75,
                    startVelocity: 38,
                    origin: { y: 0.6 },
                    colors: ['#7c9dff', '#c084fc', '#8b5cf6', '#4ade80']
                });
            }
            launch();
        })();
...
def scroll_qa_into_view_mobile(anchor_id: str) -> None:
    """모바일(<=640px) 화면에서만 지정한 앵커를 화면 맨 위로 스크롤한다. PC 화면은 영향받지 않는다."""
    components.html(f"""
        <script>
        (function() {{
            function run(tries) {{
                ...
                if (!el) {{
                    if (tries > 0) setTimeout(function() {{ run(tries - 1); }}, 40);
                    return;
                }}
...
def inject_session_keepalive() -> None:
    """비활동 상태가 길어져도 세션(단어 목록 등)이 초기화되지 않도록 주기적으로 백그라운드 핑을 보낸다."""
    components.html("""
        <script>
...
def get_session_rng() -> random.Random:
    ...
    여러 사용자가 동시에 접속해도 전역 random 상태를 공유하지 않으므로
    한 사용자의 셔플/출제 순서가 다른 사용자에게 영향을 주지 않는다.
    """
...
def render_active_files_banner() -> None:
    """학습/연습/시험/지문 화면에 현재 로드된 파일 목록을 보여준다."""
...
# ---------------------------
def render_date_calendar(available_dates: dict) -> None:
    """실제 달력처럼 요일에 맞춰 날짜를 배치하고, 파일이 있는 날짜만 클릭해서 선택/해제할 수 있다.

    선택된 날짜는 st.session_state.selected_dates(문자열 집합, 'YYYY-MM-DD')에 저장되며,
    체크박스(value= 재지정) 방식의 버그를 피하기 위해 버튼 클릭으로 직접 토글한다.
    """
    ...
    with nav1:
        if st.button("◀ 이전 달", use_container_width=True, key="cal_prev_month"):
            y, m = st.session_state.sidebar_cal_y, st.session_state.sidebar_cal_m
            m -= 1
            if m == 0:
                m, y = 12, y - 1
            st.session_state.sidebar_cal_y, st.session_state.sidebar_cal_m = y, m
            st.rerun()
    with nav2:
        if st.button("오늘로 이동", use_container_width=True, key="cal_goto_today"):
            st.session_state.sidebar_cal_y = today_date.year
            st.session_state.sidebar_cal_m = today_date.month
            st.rerun()
    with nav3:
        if st.button("다음 달 ▶", use_container_width=True, key="cal_next_month"):
            y, m = st.session_state.sidebar_cal_y, st.session_state.sidebar_cal_m
            m += 1
            if m == 13:
                m, y = 1, y + 1
    ...
    st.markdown(f"<div style='text-align:center;font-weight:800;margin:4px 0 8px 0;'>{y}년 {m}월</div>", unsafe_allow_html=True)

    cal = calendar.monthcalendar(y, m)
    weekdays = ["월", "화", "수", "목", "금", "토", "일"]

    grid_box = st.container(key="calendar_grid_box")
    with grid_box:
        w_cols = st.columns(7)
        for i, wd in enumerate(weekdays):
            color = "#ff8a8a" if i == 6 else ("#8ab4ff" if i == 5 else "inherit")
            w_cols[i].markdown(
                f"<div style='text-align:center;font-weight:700;font-size:0.78rem;color:{color};'>{wd}</div>",
                unsafe_allow_html=True
            )
        ...
        for w_idx, week in enumerate(cal):
            ...
            for i, day in enumerate(week):
                ...
                dt = date(y, m, day)
                dt_str = dt.isoformat()
                is_today = (dt == today_date)
                has_file = dt in available_dates
                is_selected = dt_str in st.session_state.selected_dates
                ...
                if has_file:
                    if is_selected:
                        ...
                        label = f"🔹{day}"
                    if d_cols[i].button(label, key=f"cal_btn_{dt_str}", use_container_width=True):
                        if is_selected:
                            st.session_state.selected_dates.discard(dt_str)
                        else:
                            st.session_state.selected_dates.add(dt_str)
                        st.rerun()
                else:
                    label = f"({day})" if is_today else str(day)
                    d_cols[i].markdown(
                        f"<div style='text-align:center;color:#6b6b75;font-size:0.72rem;padding:6px 0;'>{label}</div>",
                        unsafe_allow_html=True
                    )

    st.caption("🔹 파일 있음 · ⭐ 오늘 · ✅ 선택됨")

    st.write("")
    week_chk_col, month_chk_col = st.columns(2)
    with week_chk_col:
        week_checked = st.checkbox("📅 이번주 선택", key="chk_this_week")
    with month_chk_col:
        month_checked = st.checkbox("🗓️ 이번달 선택", key="chk_this_month")
    ...
    if week_checked:
        ...
        for i in range(7):
            ...
            if d in available_dates:
                st.session_state.selected_dates.add(d.isoformat())

    if month_checked:
        for d in available_dates:
            if d.year == today_date.year and d.month == today_date.month:
                st.session_state.selected_dates.add(d.isoformat())

    if st.button("달력 선택 모두 해제", use_container_width=True, key="cal_clear_all"):
        st.session_state.selected_dates = set()
        st.session_state.chk_this_week = False
        st.session_state.chk_this_month = False
        st.rerun()
...
def render_sidebar() -> list:
    ...
    with st.sidebar:
        ...
        with st.expander("🗓️ 달력에서 날짜로 찾기", expanded=False):
            render_date_calendar(available_dates)

        selected_dates_set = st.session_state.get("selected_dates", set())
        selected_cal_files = []
        for dt, files in available_dates.items():
            if dt.isoformat() in selected_dates_set:
        ...
        st.write("**3. 최종 파일 확인 및 선택**")

        file_labels = [f["label"] for f in all_files]

        # 위젯 key(filechk_{lbl}_widget)를 진짜 상태 저장소로 사용한다.
        # 새로 나타난 파일은 기본값 False(전체 해제)로 시작한다.
        widget_keys = [f"filechk_{lbl}_widget" for lbl in file_labels]
        for wk in widget_keys:
            if wk not in st.session_state:
                st.session_state[wk] = False

        sel_all_col, desel_all_col = st.columns(2)
        with sel_all_col:
        ...
        for lbl in file_labels:
            wk = f"filechk_{lbl}_widget"
            st.checkbox(lbl, key=wk)
        st.markdown('</div>', unsafe_allow_html=True)

        selected_labels = [
            lbl for lbl in file_labels
            if st.session_state.get(f"filechk_{lbl}_widget", False)
        ]
...
def load_data(selected_files: list, is_script: bool = False) -> bool:
    ...
    동시 접속 사용자가 많아도 서로 영향을 주지 않도록:
    - 모든 상태는 st.session_state(사용자별 독립 저장소)에만 저장한다.
    - 셔플에는 전역 random 모듈 대신 세션 전용 random.Random 인스턴스를 사용해
      여러 사용자가 동시에 섞기를 실행해도 서로의 순서에 영향을 주지 않는다.
    """
...
def render_study_part() -> None:
    ...
    if st.session_state.is_studying:
        if st.session_state.study_index < len(st.session_state.words):
            ...
            st.markdown('<div id="study_qa_anchor"></div>', unsafe_allow_html=True)
            scroll_qa_into_view_mobile("study_qa_anchor")

            with mobile_stack_container("study_nav_btns"):
                c1, c2 = st.columns(2)
                with c1:
...
def render_practice_part() -> None:
    ...
    if st.session_state.is_practicing and st.session_state.current_practice_word:
        ...
        st.markdown('<div id="practice_qa_anchor"></div>', unsafe_allow_html=True)
        scroll_qa_into_view_mobile("practice_qa_anchor")

        with mobile_stack_container("practice_action_btns"):
        ...
        with mobile_stack_container("practice_score_btns"):
        ...
        card_html = f"<div class='study-card qa-compact'><div class='test-question'>Q: {q_text}</div>"
        if is_ans_shown:
...
def render_exam_part() -> None:
    st.header("🎯 시험 파트")
    st.caption("단축키: [스페이스바] 정답 확인(힌트도 함께 표시) · [Z] O 맞음 · [X] X 틀림")
    selected_files = render_sidebar()
    ...
    if st.session_state.is_examining and st.session_state.current_exam_word:
        ...
        st.markdown('<div id="exam_qa_anchor"></div>', unsafe_allow_html=True)
        scroll_qa_into_view_mobile("exam_qa_anchor")

        with mobile_stack_container("exam_action_btns"):
        ...
        if st.button("🔁 같은 조건으로 다시 시험", use_container_width=True):
...
def render_script_part() -> None:
    ...
    if st.session_state.is_scripting:
        if st.session_state.script_index < len(st.session_state.script_lines):
            ...
            st.markdown('<div id="script_qa_anchor"></div>', unsafe_allow_html=True)
            scroll_qa_into_view_mobile("script_qa_anchor")

            with mobile_stack_container("script_nav_btns"):
                c1, c2 = st.columns(2)
                with c1:
    ...
    st.markdown("""
        ...
        <div style="text-align:center; margin-bottom: 6px;">
            <span style="
                ...
                background: linear-gradient(90deg,#7c9dff,#c084fc,#7c9dff);
                background-size: 200% auto;
                -webkit-background-clip: text; background-clip: text; -webkit-text-fill-color: transparent;
                animation: gradientShift 4s ease infinite; display:inline-block;
            ">📚 단어 암기 프로그램</span>
        </div>
        <div style="text-align:center; color:#9a9aa2; font-size:0.9rem; margin-bottom:14px;">
            매일 조금씩, 확실하게 외우기 ✨
        </div>
    """, unsafe_allow_html=True)
