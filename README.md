...
### マイクロラーニング — 15語ラウンドとよく間違える単語

一度に数百語をまとめて解かせると、途中で疲れてしまいがちです。そこで練習を15語単位の
「ラウンド」に分割しました。15語終えるたびに短い完了画面が表示され、画面上部の進捗
バー(ジェリースタイル)も全体ではなくそのラウンド基準で満ちていきます。3回以上
曖昧・分からないと採点された単語は、正解画面に「よく間違える単語」バッジが付き、
どの単語が特に覚えにくいかすぐに分かります。

### 間隔反復(SRS) — 今日の復習

完璧/少し分かると採点した単語には、SuperMemo-2の考え方を参考にした間隔反復
アルゴリズムを適用します。正解するたびに次の復習までの間隔が1日 → 3日 → 7日 ...と
広がっていき、間違えると間隔は0にリセットされます。その間隔が来た単語は「復習」画面の
「今日の復習」カードに集まり、今すぐ見直すべき単語だけを選んで練習を始められます。
...
- ドロップダウンで組み合わせを選び**[この学習をもう一度]**を押すと、そのときの
  ...
  画面まで進む — 今すぐ再開できる保存中の進捗と完全に同じ組み合わせなら、最初から
  シャッフルし直さずキューと完了数をそのまま引き継ぎます。

ファイルの組み合わせごとの進捗記録は(ユーザー番号、パート、ファイルの組み合わせ)
単位で別途保存され、セッションが終わった後も最後の進捗率と学習時刻が残ります。
ネットワークが一時的に切れても、この端末のlocalStorageに即座に補助保存され、
接続が復旧すればサーバーと自動的にマージされます。

### 他の暗記アプリを参考にした機能

プロトタイプの問題点を解決した後、Anki・Quizlet・Duolingoなど代表的な暗記/語学アプリ
も調べ、word_appの構造に合う形で機能を追加しました。
...
- **マッチングゲーム** — Quizletの「Match」ゲームに相当します。選んだファイルから
  ...
  されます。「その他」メニューの中ではなく、下部タブナビゲーションに練習と試験の
  間の専用タブとして配置しています。
- **発音を聞く** — 学習・練習画面の単語横のスピーカーアイコンを押すと、端末内蔵の
  音声合成でその単語の発音を聞けます。外部APIなし、ブラウザ標準機能だけで動作します。
- **メモ帳(memo)** — 学習・練習画面右下の「memo」ボタンから、手書きメモを開けます。
  Apple Pencil(またはマウス)でしか描画されず指のタッチは無視されるので、iPadで
  手のひらを画面に置いても描かれません。右下の角をドラッグしてサイズ変更でき、
  「閉じる」を押しても内容は消えず、再度memoボタンを押せば続きを書けます。

UIは絵文字を使わず、テキストとアイコンだけでシンプルにまとめています。既定のテーマも
ライトではなくダークです(設定でいつでも切り替え可能)。
...
### 마이크로 러닝 — 15개 라운드와 자주 틀리는 단어

한 번에 수백 개를 몰아서 풀게 하면 중간에 지치기 쉽습니다. 그래서 연습을 15개 단위
"라운드"로 쪼갰습니다. 15개를 채울 때마다 짧은 완료 화면이 뜨고, 화면 상단 진행률
바(젤리 스타일)도 전체가 아니라 이번 라운드 기준으로 다시 채워집니다. 세 번 이상
헷갈림·모름으로 채점된 단어는 정답 화면에 "자주 틀리는 단어" 배지가 붙어서, 어떤
단어가 특히 안 외워지는지 바로 알 수 있습니다.

### 간격 반복(SRS) — 오늘의 복습

완벽함·조금 앎으로 채점한 단어에는 SuperMemo-2 방식에서 착안한 간격 반복 알고리즘을
적용합니다. 맞힐 때마다 다음 복습까지의 간격이 1일 → 3일 → 7일 ...로 점점 늘어나고,
틀리면 간격이 0으로 리셋됩니다. 그 간격이 다 된 단어는 "복습" 화면의 "오늘의 복습"
카드에 모여서, 지금 다시 봐야 할 단어만 골라 바로 연습을 시작할 수 있습니다.
...
- 드롭다운에서 조합을 고르고 **[이 학습 다시 하기]**를 누르면, 그때 썼던 파일 선택과
  모드(이름만/뜻만/랜덤)가 그대로 복원되면서 자동으로 단어 화면까지 진입 — 지금 이어서
  할 수 있는 저장된 진행과 정확히 같은 조합이면, 처음부터 다시 섞지 않고 큐와 완료
  개수를 그대로 이어갑니다.

파일 조합별 진행 기록은 (사용자 번호, 파트, 파일 조합) 단위로 별도 저장되어, 세션이
끝난 뒤에도 마지막 진행률과 학습 시각이 남습니다. 네트워크가 일시적으로 끊겨도 이
기기의 localStorage에 즉시 보조 저장되고, 연결이 복구되면 서버와 자동으로 병합됩니다.

### 다른 암기 앱을 참고해 추가한 기능

프로토타입의 문제점을 해결한 뒤, Anki·Quizlet·Duolingo 같은 대표적인 암기/어학 앱들도
조사해서 word_app 구조에 맞는 형태로 기능을 추가했습니다.
...
- **매칭 게임** — Quizlet의 "Match" 게임에 해당합니다. 선택한 파일에서 무작위로 6쌍
  ...
  그 조합의 기존 최고 기록보다 빠르면 새 기록으로 저장합니다. "더보기" 메뉴 안이
  아니라 하단 탭 내비게이션에 연습과 시험 사이 전용 탭으로 배치했습니다.
- **발음 듣기** — 학습·연습 화면 단어 옆 스피커 아이콘을 누르면, 기기에 내장된 음성
  합성으로 그 단어의 발음을 들을 수 있습니다. 외부 API 없이 브라우저 기본 기능만으로
  동작합니다.
- **메모장(memo)** — 학습·연습 화면 오른쪽 아래 "memo" 버튼으로 손글씨 메모장을 열
  수 있습니다. 애플펜슬(또는 마우스)로만 그려지고 손가락 터치는 무시되어, 아이패드에서
  손바닥을 화면에 대고 써도 그려지지 않습니다. 오른쪽 아래 모서리를 끌어 크기를 바꿀
  수 있고, "닫기"를 눌러도 내용이 지워지지 않아 memo 버튼을 다시 누르면 이어서 쓸 수
  있습니다.

UI에서는 이모티콘을 쓰지 않고 텍스트와 아이콘만으로 간결하게 구성했습니다. 기본
테마도 라이트가 아니라 다크입니다(설정에서 언제든 전환 가능).
