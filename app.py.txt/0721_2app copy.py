...
def parse_script_text(text):
    """지문 한 줄 파싱: 빈 줄을 제외한 모든 줄을 각각의 문장으로 인식"""
    normalized = text.replace("\r\n", "\n")
    lines = normalized.split("\n")
...
def render_wordbook_part():
    ...
    c1, c2 = st.columns(2)
    with c1: main_cat = st.selectbox("대분류 선택", categories, key="wb_main_cat")
    with c2: 
        sub_folders = get_subfolders(main_cat)
        sub_cat = st.selectbox("하위 폴더 선택", sub_folders, key="wb_sub_cat") if sub_folders else None
...
def main():
    ...
    # 지문 외우기 파트가 라디오 버튼(메뉴)에 추가되었습니다.
