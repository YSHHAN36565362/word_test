...
def apply_global_style() -> None:
    ...
    st.markdown(f"""
        ...
        /* 문제+정답+힌트를 한 카드 안에 압축 배치(스크롤 없이 한 화면에 보이도록) */
        ...
        .st-key-calendar_grid_box div[data-testid="stCheckbox"] {{
            display: flex; justify-content: center;
        }}
        .st-key-calendar_grid_box div[data-testid="stCheckbox"] label {{
            padding: 2px 0 !important;
        }}
        .st-key-calendar_grid_box div[data-testid="stCheckbox"] label p {{
...
def get_session_rng() -> random.Random:
    ...
    20명 등 여러 사용자가 동시에 접속해도 전역 random 상태를 공유하지 않으므로
...
def render_date_calendar(available_dates: dict) -> None:
    """모바일에서도 7칸 격자가 절대 줄바꿈되지 않는 컴팩트 달력을 렌더링한다."""
            ...
            for i, day in enumerate(week):
                if day == 0:
                    ...
                    if dt in available_dates:
                        ...
                        d_cols[i].checkbox(label, key=chk_key, label_visibility="visible")
                    else:
                        d_cols[i].markdown(
                            f"<div style='text-align:center;color:#6b6b75;font-size:0.75rem;padding:4px 0;'>{day}</div>",
                            unsafe_allow_html=True
                        )
...
def render_sidebar() -> list:
    ...
    with st.sidebar:
        ...
        valid_keys = {f"filechk_{lbl}" for lbl in file_labels}
        for k in list(st.session_state.sidebar_file_checks.keys()):
            if k not in valid_keys:
                del st.session_state.sidebar_file_checks[k]
        ...
        with sel_all_col:
            if st.button("전체 선택", use_container_width=True, key="select_all_files_btn"):
                for lbl in file_labels:
                    st.session_state.sidebar_file_checks[f"filechk_{lbl}"] = True
                st.rerun()
        with desel_all_col:
            if st.button("전체 해제", use_container_width=True, key="deselect_all_files_btn"):
                for lbl in file_labels:
                    st.session_state.sidebar_file_checks[f"filechk_{lbl}"] = False
                st.rerun()

        st.markdown('<div class="file-check-row">', unsafe_allow_html=True)
        for lbl in file_labels:
            chk_key = f"filechk_{lbl}"
            if chk_key not in st.session_state.sidebar_file_checks:
                st.session_state.sidebar_file_checks[chk_key] = True
            st.session_state.sidebar_file_checks[chk_key] = st.checkbox(
                lbl, value=st.session_state.sidebar_file_checks[chk_key], key=chk_key + "_widget"
            )
        st.markdown('</div>', unsafe_allow_html=True)

        selected_labels = [
            lbl for lbl in file_labels
            if st.session_state.sidebar_file_checks.get(f"filechk_{lbl}", False)
        ]
