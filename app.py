...
def init_session_state() -> None:
    ...
    defaults = {
        "words": [],
        "current_files_label": [],   # 현재 로드된 파일 라벨 목록 (진행 중 화면에 표시)
...
def apply_global_style() -> None:
    ...
    st.markdown(f"""
        ...
        /* 최종 파일 선택 체크박스 목록: 한 줄에 파일명 전체가 보이도록 좌측 정렬 */
        ...
        /* 완료/성공 메시지 살짝 튀어오르는 애니메이션 */
        div[data-testid="stAlert"] {{
            animation: popIn 0.4s cubic-bezier(0.34, 1.56, 0.64, 1) both;
            border-radius: 14px !important;
        }}

        /* ---------- 배경 블롭(깊이감 있는 은은한 그라디언트 원) ---------- */
        .bg-blob {{
            position: fixed;
            border-radius: 50%;
            filter: blur(60px);
...
def render_active_files_banner() -> None:
    """학습/연습/시험/지문 화면 상단에 현재 로드된 파일 목록을 보여준다."""
...
def render_sidebar() -> list:
    ...
    with st.sidebar:
        ...
        # --- 최종 파일 선택: 체크박스 목록(한 줄에 파일명 전체 표시) ---
        ...
        with mobile_stack_container("practice_action_btns"):
            ...
            with btn1:
                if st.button("정답 확인", use_container_width=True):
                    ...
                    st.session_state.practice_show_hint = has_hint  # 정답 확인 시 힌트도 함께 표시
                    st.rerun()
            with btn2:
                if st.button("힌트 보기", use_container_width=True, disabled=not has_hint):
