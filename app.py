...
# ---------------------------
# 2. 글로벌 CSS 스타일 (차분하고 학습에 방해되지 않는 디자인)
# ---------------------------
def apply_global_style() -> None:
    ...
    st.markdown(f"""
        ...
        /* 모든 화면 크기에서 자연스럽게 줄어들고 늘어나는 반응형 기본 크기 */
        ...
        /* ---------- 학습 카드: 은은하고 정적인 디자인, 불빛/펄스 없음 ---------- */
        .study-card {{
            background: {card_bg};
            border: 1px solid {border_color};
            border-radius: 14px;
            padding: clamp(20px, 4vw, 34px) clamp(16px, 3vw, 26px);
            margin: 16px 0;
        ...
        .qa-compact {{ padding: clamp(18px, 3vw, 26px) clamp(16px, 3vw, 22px) !important; margin: 8px 0 !important; }}
        .qa-compact .test-question {{ padding: 4px 4px 2px 4px !important; }}
        ...
        .test-question {{
            font-size: clamp(1.6rem, 4.2vw, 2.4rem) !important; color: {text_color}; text-align: center; padding: 14px 8px 6px 8px;
            font-weight: 700; word-break: break-word;
        }}
        .test-answer {{
            font-size: clamp(1.1rem, 2vw, 1.4rem) !important; text-align: center; color: {ans_color}; font-weight: 700; margin: 2px 0 16px 0;
        }}

        .progress-caption {{ text-align: center; color: {muted_color}; margin-top: 6px; font-size: 0.9rem; }}
...
def fire_confetti() -> None:
    """완료 화면에서만 짧게 표시되는 축하 효과. 학습 중에는 호출되지 않는다."""
    components.html("""
...
def scroll_qa_into_view_mobile(anchor_id: str) -> None:
    """모바일(<=700px) 화면에서만 지정한 앵커를 화면 맨 위로 스크롤한다. PC 화면은 영향받지 않는다."""
    components.html(f"""
        <script>
        (function() {{
            function run(tries) {{
...
# ---------------------------
# 5. UI - 사이드바 (카테고리 기반 파일 선택)
# ---------------------------
def render_sidebar() -> list:
    """대분류 -> 하위 카테고리(예: N2, N3) -> 파일 순으로 선택한다.

    각 하위 카테고리 그룹 안에서 전체 선택/전체 해제가 독립적으로 동작하며,
    전체 파일에 대한 일괄 선택/해제 버튼도 별도로 제공한다.
    """
    with st.sidebar:
...
def load_data(selected_files: list, is_script: bool = False) -> bool:
    ...
    - 셔플에는 전역 random 대신 세션 전용 random.Random 인스턴스를 사용한다.
    """
...
def render_exam_part() -> None:
    ...
    if st.session_state.is_examining and st.session_state.current_exam_word:
        ...
        with mobile_stack_container("exam_action_btns"):
            ...
            with c2:
                if st.button("맞음", disabled=not is_ans_shown, use_container_width=True): next_exam(True); st.rerun()
            with c3:
                if st.button("틀림", disabled=not is_ans_shown, use_container_width=True): next_exam(False); st.rerun()
            with c4:
